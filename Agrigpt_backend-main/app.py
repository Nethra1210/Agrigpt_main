...
pest_simulation_tool = StructuredTool.from_function(
    func=simulate_pests,
    name="simulate_pests",
    description="Simulates pest and disease activity for a given crop and location. Pass crop_name and optionally location."
)

government_schemes_tool = StructuredTool.from_function(
    func=get_government_schemes,
    name="government_schemes",
    description="Retrieves information on agricultural government schemes and subsidies. Pass state name."
)
...
@app.post("/set-keys", summary="Set API Keys Dynamically", tags=["Settings"])
async def set_keys(request: SetKeysRequest):
    global GOOGLE_API_KEY, app_agent
    ...
    if request.google_api_key:
        ...
        # Rebuild agent to pick up the new LLM key
        app_agent = build_agent()
        msg.append("Google API Key updated and agent rebuilt.")
    ...
    if request.langsmith_api_key:
        ...
        os.environ["LANGCHAIN_PROJECT"] = "agrigpt-backend-agent"
        msg.append("LangSmith Observability is now configured for future requests.")
